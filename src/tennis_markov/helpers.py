import sympy as sp
from sympy import symbols, Matrix, simplify, factor, latex
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import Rectangle


def create_symbolic_game_tennis_matrix():
    """
...
def create_symbolic_set_tennis_matrix():
    """
...
def create_symbolic_match_tennis_matrix(best_of=5):
    """
...
def solve_win_probability(P, num_absorbing_states, level):
    """
    ...
    print(f"B matrix:\n{B}\n Has shape:\n{B.shape}")
    print("The rows in B denote the starting state, and the columns are the absorbing states (Player 1 wins or Player 2 wins). So the entry [0,0] is the probability that starting the game at 0-0, player 1 will win the game.") 
...
def simplify_expression(expr, level, max_steps=5):
    """
...
def plot_probabilities(symb_expr, level):
    if level == "game":
...
def write_latex(simplified_prob, level):
...
def plot_probability_match_given_point(game_expr, set_expr, match_expr):     
    ...
    ax.plot(x_vals, x_vals, color=colors['identity'], linestyle='--', 
            linewidth=1.5, alpha=0.4, label='$P_{point} = P_{outcome}$')
    
    # Plot main curves with professional styling
    ax.plot(x_vals, y_vals_game, color=colors['game'], linewidth=3, 
            label='Game probability', zorder=3)
    ax.plot(x_vals, y_vals_set, color=colors['set'], linewidth=3, 
            label='Set probability', zorder=3)
    ax.plot(x_vals, y_vals_match, color=colors['match'], linewidth=3, 
            label='Match probability', zorder=3)
    
    # Annotation points with enhanced styling
    annotation_points = [0.5, 0.55, 0.6]
    functions = [f, f_g, f_s]
    curve_colors = [colors['game'], colors['set'], colors['match']]
    curve_labels = ['Game', 'Set', 'Match']
    
    # Add vertical reference lines
    for p_val in annotation_points:
        ax.axvline(x=p_val, color=colors['reference'], linestyle=':', 
    ...
    table_bg = Rectangle((table_x, table_y - table_height), table_width, table_height,
                        transform=ax.transAxes, facecolor='white', 
                        edgecolor=colors['annotation'], linewidth=1.5, alpha=0.95)
    ...
    for i, (header, color) in enumerate(zip(headers, header_colors)):
        ax.text(table_x + 0.02 + i*0.075, table_y - 0.03, header, 
                transform=ax.transAxes, fontsize=11, fontweight='bold',
                color=color, ha='center')
    
    # Add horizontal line under headers
    ax.plot([table_x + 0.01, table_x + table_width - 0.01], 
            [table_y - 0.05, table_y - 0.05], 
            transform=ax.transAxes, color=colors['annotation'], linewidth=1)
    ...
    for p_val in annotation_points:
        for func, color in zip(functions, curve_colors):
            y_val = func(p_val)
            ax.plot(p_val, y_val, 'o', color=color, markersize=7, 
    ...
    ax.set_title('Tennis Win Probabilities: From Points to Matches', 
    ...
    legend = ax.legend(loc='center right', bbox_to_anchor=(0.98, 0.3),
...
def plot_probability_match_split_panel(game_expr, set_expr, match_expr):
    """
    ...
    ax_main.plot(x_vals, y_vals_match, color=colors['match'], linewidth=3, label='Match')
    ...
    for p_val in annotation_points:
        ...
        for func, color in zip([f, f_g, f_s], colors.values()):
            y_val = func(p_val)
            ax_main.plot(p_val, y_val, 'o', color=color, markersize=8, 
...
def save_publication_plots(game_expr, set_expr, match_expr, filename_base='tennis_probabilities'):
    ...
    formats = ['png', 'pdf']
    for fmt in formats:
        fig1.savefig(f'{filename_base}_main.{fmt}', 
    ...
    for fmt in formats:
        fig2.savefig(f'{filename_base}_split.{fmt}', 
                    format=fmt, dpi=300, bbox_inches='tight',
                    facecolor='white', edgecolor='none')
